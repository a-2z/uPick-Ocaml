(*Test Plan: We used a mix of manual and automated testing to ensure the 
  ...
  *****************Automated (Dbquery, Search, Passwords)***********************
  ...
  3  terfacing get and set functions in the Dbquery module, we thoroughly 
...
let try_get ?succ:(b = true) name get_func id = 
  let success = begin
    try 
...
let friends_6 = ignore (add_friends 1 2)
let friends_7 = ignore (add_friends 2 3)
let friends_8 = ignore (add_friends 5 13)
...
let del_restriction (ad_id, rest_id) = remove_restrictions_index 
    ad_id rest_id
...
let cuisines_test = [
  ...
  try_get ~succ:true "ensures that the first cuisine is inserted correctly" 
    ins_cuisine (1, 55, "Italian"); 
  test_passes ~succ:false "ensures that invalid cuisine cannot be added" 
    ins_cuisine (1, 25, "");
  try_get ~succ:true "test that admin can delete cuisine" 
    del_cuisine (1, 55);
...
let delete_from_group_test = [
  try_get ~succ:true 
    "kick first member of group" kick_member (1, 3, 7);
  test_passes ~succ:false 
    "kick somone not in group" kick_member (1, 2, 7);
  test_passes ~succ:false 
    "cannot kick host from group" kick_member (1, 7, 7);
  test_passes ~succ:false 
    "kick the maximum user_id from the group" kick_member (2, max_int, 7);
  test_passes ~succ:false
...
let reassign_host_test = [
  try_get ~succ:true "ensures that original host is correct" is_host (1, 7);
  try_get ~succ:false "cannot return invalid host" is_host (2, 0);
  try_get ~succ:false "cannot get host of invalid group" is_host (0, 2);
  try_get ~succ:true 
    "successfully change host" change_host (1, 2, 7);
  test_passes ~succ:false 
    "ensures cannot change host of invalid group" change_host (0, 2, 5);
  test_passes ~succ:false
    "cannot change host if user does not exist" change_host (3, 10, 6);
  test_passes ~succ:false 
    "cannot change host if host_id is invalid" change_host (4, 2, 5);
  test_passes ~succ:false
    "cannot assign host if user_id is already host" change_host (4, 5, 5);
]

let rests_exist ?survey:(s = true) ?succ:(b = false) name grp_id = 
  let restaurants =   
  ...
  let result = 
    ...
    | Some _ -> true
    | None -> false in 
  name >:: fun _ -> assert_bool "result was not determined" (result = b)
...
(* let test_visited ?succ:(b = true) name str_gid top = 
   let comp = if b then ( = ) else ( <> ) in
   test_equal ~compare: comp name (visited_entries str_gid) top *)

(* let top_visited_1 = top_visited ["1"; "2"]; *)

(* let visited_test = [
   test_visited ~succ:false "valid visited restaurant"   *)
