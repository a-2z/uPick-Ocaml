...
let make_response ?msg:(m = "") = function
  | Some id -> respond' 
                 (`Json (Ezjsonm.from_string 
                           ({|{"success": true, "id": |} ^
                            Int64.to_string id ^ "}")))
  | None -> if m = "" 
    then respond' (`Json (Ezjsonm.from_string 
                            {|{"success": false }|}))  
    else respond' (`Json (Ezjsonm.from_string 
                            (Printf.sprintf                                                                    
                               {|{"success": false, "data": "%s"}|} m)))
...
let load_json_login req ins_func inserter =
  ...
  try 
    ...
    login json;
    Lwt.return (inserter (from_string body) ins_func) >>= 
    fun row_id -> make_response row_id
  with 
  | Login_failure _ -> 
    Lwt.return None >>= 
    fun row_id -> make_response row_id ~msg:"Invalid username or password." 
      ...
      try 
        ...
        (`Json (Ezjsonm.from_string 
                  (Printf.sprintf {|{"success": true, "data": "%s"}|} 
                     restriction)))
        |> respond'
      with _ -> respond' (`Json (Ezjsonm.from_string {|{"success": false}|})));
